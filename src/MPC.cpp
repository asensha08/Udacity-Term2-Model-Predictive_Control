...
/**to establish when one variable starts and another ends to make our lifes easier.
*/
size_t x_start = 0; 
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(unsigned int i=0; i<N; i++){
      fg[0]+= 30*CppAD::pow(vars[cte_start+i],2);
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      AD<double> psides0 = CppAD::atan(coeffs[1] + (2*coeffs[2]*x0) + (3*coeffs[3]*CppAD::pow(x0,2)));
       ...
       //Setup of the rest of the model constraints
      fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
      fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
      fg[1 + psi_start + t]=psi1 - (psi0 - v0*delta0/Lf *dt);// Chnaged the sign as per the simulator sign convenstions
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Defining the upper and lower limits  for all the non
  //actuator states
